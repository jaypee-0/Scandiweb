...
class Card extends Component {
  ...
  render() {
    const { item, itemNames, addItem, addQuantity, getOccurrence, quantities, productId, dataInStock, thumbnail, productTitle, productPrice, category } = this.props;
    const cat = category.products.map((data) => data.category)[0]
    //console.log(item?.attributes);
    //console.log([item.attributes[0]?.items[0]]);
    return (
      <Link to={`/${cat}/${productId}`}>
        <div data-instock={dataInStock} className="product-card">
          <div className="img-text">
            <img src={thumbnail} alt={productTitle}></img>
            <div className="title-price">
              <h4 style={{marginBottom: 0}}>{productTitle}</h4>
              <p style={{marginTop: 0}}>{productPrice}</p>
            </div>
          </div>
            ...
            <svg
              onClick={() => {
                if (item.attributes.length === 0) {
                 ...
                 //addItem([[item], [], [], [item.name]], item.name);
                }
              }}
              width="74"
              height="74"
